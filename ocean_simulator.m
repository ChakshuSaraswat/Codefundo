...
% function h = ocean_simulator(~)
%
% This is a Mini GUI to experiment a few parameters used to create a wave simulation and to study the wave characteristics and also to differentiate between
%Tsunami and normal waves and give rise to an early warning system



%% // Default parameters--- Parameters that would affect the wave 
...
param.xLim = [-10 10] ;     %// domain limits in  X
param.yLim = [-10 10] ;     %// domain limits in  Y
...
%NOW WE START TO INITIALISE FOR THE WAVE AND INTRODUCING GRID IN IT 
...
% Third output parameter is optional (easy to recalculate anyway)
    ...
    rng(param.rng);  %// setting seed for random numbers i.e to generate random numbers that will give us the dataset for the run time 
    ...
    P = Living_Code(Kx, Ky, [windx , windy], param.windSpeed, param.A, param.g) ;
...
function P = Living_Code(Kx, Ky, windDir, windSpeed, A, g)
...
function animate_wave(h,~)
    ...
    Z = calc_wave( H0,W,time,Grid_Sign ) ;
    hs.ZData = Z ;
    fname=strcat('Output',num2str(time),'.jpg');
    Z=normalize(Z,'range');
    save (fname,'Z')
    imwrite(Z,fname);   
